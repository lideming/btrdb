...
export type Deferred<T> = Promise<T> & { resolve(val: T): void; reject(err: any): void; };

export function deferred<T>(): Deferred<T> {
    let resolve, reject;
    var prom = new Promise((r, rej) => { resolve = r, reject = rej; }) as any;
...
export class OneWriterLock {
    ...
    exitWriter() {
        if (this.writers != 1 || this.readers != 0) throw new Error('BUG, ' + this.writers + ', ' + this.readers);
        this.writers--;
        if (Math.random() < 0.5) {
